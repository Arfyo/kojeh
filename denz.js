...
│
└───「 \`\`\`${NamaBot}\`\`\` 」`
sendButLocation(from, `${menu}`, "*_ROBOTZ_*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`command`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`script`,buttonText:{displayText:'SOURCE CODE'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
stod = `${sender}`
       stst = await denz.getStatus(`${sender.split('@')[0]}@c.us`)
       nama = '👽ROBOTZ👽'
       ownerJid = "62895410396931@s.whatsapp.net"
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
menunya = `╭─❒ 「 Bot Info 」 ❒
├ _Creator : @${dtod.split('@')[0]}_
...
break
		...
		anu =`╭─❒ SCRIPT
│◦➛Base : Denis Putra
│◦➛Recode : Yuzzu Kamyaka
│◦➛Recode: Arfy
└──────[ GITHUB ]──────❒
  │◦➛Script Ory :
  │◦➛https://github.com/dcode-denpa/bitch-boot
  │◦➛Script Recode : 
  │◦➛https://github.com/YuzzuKamyaka/bitch-bot
  └──────────────────❒`
 buttons = [
